                         ...
                         // return val.getTakeTrainingList.isNotEmpty ? ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            return InkWell(
                                ...
                                child: Padding(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        SizedBox(height: size.height * 0.01 / 2),
                                        Container(
                                          decoration: BoxDecoration(color:CustomColors.kWhiteColor,border: Border.all(width: 2, color: CustomColors.kBlueColor),
                                        ...
                                        filteredList[index].isJoin==false? Card(
                                          color: CustomColors.kWhiteColor,
                                          elevation: 3,
                                          shape: OutlineInputBorder(borderRadius: BorderRadius.circular(10),borderSide: BorderSide.none),
                                          child:Padding(
                              ...
                              onTap: () {
                                int ind = filteredList.indexOf(filteredList[index], index);
                                Navigator.of(context).push(MaterialPageRoute(builder: (context) => TakeTrainingDetailScreen(trainingResponse: filteredList[index], orgID: widget.orgId!,index:ind)));
                              },
                            );
                          },
                        ) :
            ...
            FutureBuilder(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  return Consumer<TakeTrainingListProvider>(
                      builder: (context, val, child) {
                        final filteredList = val.getTakeTrainingList.where((item) => item.isJoin==false).toList();
                        return filteredList.isNotEmpty ? ListView.builder(
                          // return val.getTakeTrainingList.isNotEmpty ? ListView.builder(
                          shrinkWrap: true,
                          itemCount: filteredList.length,
            ...
            FutureBuilder(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  return Consumer<TakeTrainingListProvider>(
                      builder: (context, val, child) {
                        final filteredList = val.getTakeTrainingList.where((item) => item.isJoin==true).toList();
                        return filteredList.isNotEmpty ? ListView.builder(
                          // return val.getTakeTrainingList.isNotEmpty ? ListView.builder(
                          shrinkWrap: true,
                          itemCount: filteredList.length,
            ...
            FutureBuilder(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  return Consumer<TakeTrainingListProvider>(
                      builder: (context, val, child) {
                        final filteredList = val.getTakeTrainingList.where((item) => item.isJoin==null && item.trainerId != personId).toList();
                        return filteredList.isNotEmpty ? ListView.builder(
                          // return val.getTakeTrainingList.isNotEmpty ? ListView.builder(
                          shrinkWrap: true,
                          itemCount: filteredList.length,
                          itemBuilder: (context, index) {
                            print("Not Joined Data----->${filteredList.length}");
                            return InkWell(
                              child: Card(
                                color: CustomColors.kWhiteColor,
                                elevation: 3,
